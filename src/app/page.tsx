...
import class1 from "../../assets/1.jpeg";
import class2 from "../../assets/2.jpeg";
import class3 from "../../assets/3.jpeg";
import class4 from "../../assets/4.jpeg";
import class5 from "../../assets/5.jpeg";
import class6 from "../../assets/6.jpeg";
import class7 from "../../assets/7.jpeg";
import class8 from "../../assets/8.jpeg";
...
const classImages = { "1.jpeg": class1, "2.jpeg": class2, "3.jpeg": class3, "4.jpeg": class4, "5.jpeg": class5, "6.jpeg": class6, "7.jpeg": class7, "8.jpeg": class8, "9.jpeg": class9 };
  ...
  return (
    <>
      ...
      <main id="main">
        ...
        <section id="classes" className="section-pad bg-[#F7F8F5]"><div className="mx-auto max-w-7xl"><motion.div {...fade} className="max-w-3xl"><p className="font-bold text-[#2F6690]">{t.nav.classes}</p><h2 className="mt-3 font-heading text-4xl font-black text-[#16425B]">{t.classes.title}</h2><p className="mt-4 text-lg leading-8 text-slate-600">{t.classes.text}</p></motion.div><div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{openingClasses.map((schoolClass) => <motion.article {...fade} key={schoolClass.title} className="overflow-hidden rounded-2xl bg-white shadow-lg shadow-slate-200 transition hover:-translate-y-1 hover:shadow-xl"><div className="relative aspect-[4/3] bg-[#D9DCD6]"><Image src={classImages[schoolClass.image as keyof typeof classImages]} alt={`${schoolClass.title} 2026-2027`} fill sizes="(min-width: 1024px) 33vw, (min-width: 640px) 50vw, 100vw" className="object-cover object-top" /></div><div className="p-5"><p className="text-sm font-bold uppercase tracking-wide text-[#3A7CA5]">{schoolClass.level}</p><h3 className="mt-2 font-heading text-2xl font-black text-[#16425B]">{schoolClass.title}</h3><p className="mt-3 font-semibold text-slate-700">{t.classes.feeLabel}: <span className="text-[#2F6690]">{schoolClass.fee}</span></p><a href="#admissions" className="mt-5 inline-flex rounded-full bg-[#16425B] px-5 py-3 text-sm font-bold text-white transition hover:bg-[#2F6690]">{t.classes.viewLabel}</a></div></motion.article>)}</div></div></section>
